...
objectdef obj_Ship
{
 ...
 variable index:module ModuleList_Smart_Bomb
 ...
 {
  return ${Me.Ship.MaxTargetRange.Centi} < ${This.m_MaxTargetRange.Centi}
 }
 ...
 method UpdateModuleList()
 {
  ...
  This.ModuleList_Smart_Bomb:Clear
  ...
  do
  {
   ...
   {
    case 72
     This.ModuleList_Smart_Bomb:Insert[${Module.Value}]
     break
    ...
    case GROUP_TRACKINGCOMPUTER
     This.ModuleList_TrackingComputer:Insert[${Module.Value}]
     break
    case GROUP_GANGLINK
     This.ModuleList_GangLinks:Insert[${Module.Value}]
     break
    default
     continue
   }
  ...
  This.ModuleList_TrackingComputer:GetIterator[Module]
  if ${Module:First(exists)}
  do
  {
   UI:UpdateConsole["  Slot: ${Module.Value.ToItem.Slot}  ${Module.Value.ToItem.Name}", LOG_MINOR, 4]
 ...
 method Activate_SmartBomb()
 {
  ...
  This.ModuleList_Smart_Bomb:GetIterator[Module]
  if ${Module:First(exists)}
  {
 ...
 function WarpToBookMark(bookmark DestinationBookmark, int WRange=0)
 {
  variable int Counter
  ...
  if ${DestinationBookmark.ToEntity(exists)}
  {
   ...
   while ${DestinationBookmark.ToEntity.Distance} > ${MinWarpRange}
   {
    ...
    while !${This.WarpEntered}
    {
     DestinationBookmark:WarpTo[${WRange}]
     wait 10
    }
    call This.WarpWait
    if ${Return} == 2
 ...
 function WarpPrepare()
 {
  ...
  UI:UpdateConsole["There are ${PilotIndex.Used} pilots in system:"]

  idx:Set[${PilotIndex.Used}]

  if ${idx} > 0
  {
   do
   {
    UI:UpdateConsole["- ${PilotIndex.Get[${idx}].Name} from ${PilotIndex.Get[${idx}].Alliance}"]
   }
   while ${idx:Dec} > 0
  }
  ...
  if !${ForceReload}
  {
   ...
   do
   {
    if !${ModuleIter.Value.IsActive} && !${ModuleIter.Value.IsChangingAmmo} && !${ModuleIter.Value.IsReloadingAmmo}
    {
     ...
     {
      variable float perc = 0.2
      if ${Me.ActiveTarget.Group.Find["Battleship"](exists)}
       perc:Set[0.3]
      ...
      if ${Math.Calc[${ModuleIter.Value.CurrentCharges}/${ModuleIter.Value.MaxCharges}]} < ${perc}
      {
       ; No, reload
       NeedReload:Set[TRUE]
      }
